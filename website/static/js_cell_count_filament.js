/* ============================================================
   js_cell_count_filament.js
   Filamentous Cell Counter UX — all client-side logic.
   ...
   Full watershed segmentation runs server-side.
   ============================================================ */
...
var LS_KEYS = {
    pixel_size:       'ccf_pixel_size',
    chamber_depth:    'ccf_chamber_depth',
    minimal_diameter: 'ccf_minimal_diameter',
    threshold_filter: 'ccf_threshold_filter',
};
...
(function () {
    ...
    var depthInput = document.getElementById('chamber_depth_range');
    if (depthInput) {
        var savedDepth = localStorage.getItem(LS_KEYS.chamber_depth);
        if (savedDepth && depthInput.dataset.serverValue === undefined) {
...
(function () {
    var form = document.getElementById('cell-count-filament-form');
    if (!form) return;
...
function exportWithOverlay() {
    ...
    offscreen.toBlob(function (blob) {
        ...
        a.href = url; a.download = 'filament_count_with_corrections.png';
...
(function () {
    if (!WORKER_URL) {
        ...
        if (btn) { btn.innerHTML = '&#128065; Live count unavailable'; btn.disabled = true; }
        return;
    }
    try {
...
function getFormParams() {
    ...
    return {
        ...
        roi: currentROI,
        // Filament params (passed to server via form; ignored by worker)
...
// ── Auto-count triggered by preprocessing param changes (debounced) ───────────
...
// ── Re-run threshold comparison thumbnails if visible ─────────────────────────
...
(function () {
    ...
    // Pre-processing params → debounced live count
    var paramIds = [
        'threshold_filter',
...
function redrawAllCircles() {
    ...
    // Server-detected circles — yellow (visible on both fluorescence bg and color images)
    ctx.lineWidth = circleLineWidth || 1; ctx.strokeStyle = '#ffff00';
    for (var i = 0; i < serverCells.length; i++) {
        var c = serverCells[i];
        ctx.beginPath(); ctx.arc(c.x * sx, c.y * sy, Math.max(1, c.r * sx), 0, 2 * Math.PI); ctx.stroke();
    }

    // Manually added circles — orange
    var sl = document.getElementById('expected_cell_size_px_range');
    var r  = sl ? parseInt(sl.value) : 10;
...
function downloadXLSX() { downloadAll(); }
...
function downloadAll() {
    ...
    var summaryData = [
        ['CyanoTools — Filamentous Cell Counter Results'],
        [],
        ...
        ['Min cell diameter',      (typeof minimal_size_um_val !== 'undefined') ? minimal_size_um_val : '?', 'µm'],
    ];
    ...
    if (typeof JSZip !== 'undefined') {
        var zip = new JSZip();
        zip.file('filament_count_results.xlsx', xlsxBuf);
        ...
        if (bgImg && bgImg.src && bgImg.src.indexOf('data:image') === 0) {
            ...
            if (origB64) zip.file('counted_image.jpg', origB64, { base64: true });
        }
        ...
        var zipBaseName = (typeof upload_image_name !== 'undefined' && upload_image_name)
            ? upload_image_name + '_filament_count' : 'filament_count_results';
