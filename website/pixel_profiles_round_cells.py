from flask import Blueprint, render_template, request, flash, session, redirect, json, jsonify
from PIL import Image as im
import os, cv2, base64, io, math, time, uuid, openpyxl
import pandas as pd
import numpy as np
    ...
    if request.method == "POST":               
        ...
        if 'image' in request.files:
            ...
            if image_extension in ALLOWED_EXTENSIONS:
                ...
                a = b = int()
                Pixel_profiles = Pixel_profiles_df = Final_profiles = coordinate = []
                ...
                if 'coordinates_all_in_session' in session:    
                    ...
                    #############################################################
                    #radius
                    radius = int(list(coordinate.values())[4])                       # type: ignore
                    ...
                    for i in range(len(rough_coordinates)):
                        ...
                        if circle is not None:
                            ...
                            for pt in circle[0, :]: # type: ignore
                            ...
                            detected_cell_radius = circle[0][0][2] # type: ignore
                            ...
                            detected_cell_center = (circle[0][0][0],circle[0][0][1]) # type: ignore
                            detected_cell_edge = (circle[0][0][0],(circle[0][0][1]+detected_cell_radius)) # type: ignore
                            ...
                            for i in range(angles_count):
                                ...
                                profile = profile_line(img_orig, start, end, linewidth=1, order=1) # type: ignore
                                ...
                                for i in range(profile.shape[0]): # type: ignore
                                    intensities = tuple([cell_number,angle,i+1,profile[i][0]]) # type: ignore
                ...
                xlsx_full_path = os.path.join(f'{upload_folder}/{_xlsx_prefix}_{image_name}_pixel_profiles.xlsx')
                ...
                xlsx_file_path = f'uploads/{_xlsx_prefix}_{image_name}_pixel_profiles.xlsx'
                ################################################
